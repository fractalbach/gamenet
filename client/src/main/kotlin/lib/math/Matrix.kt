...
data class Mat4(
    ...
    val rotation: Float3
        get() {
            ...
            return when {
                z.y <= -1.0f -> Float3(degrees(-HALF_PI_32), 0.0f, degrees(atan2( x.z,  y.z)))
                z.y >=  1.0f -> Float3(degrees(HALF_PI_32), 0.0f, degrees(atan2(-x.z, -y.z)))
                else -> Float3(
...
fun rotation(d: Float3): Mat4 {
    val r = transform(d, { x: Float -> radians(x)})
    val c = transform(r, { x: Float -> cos(x.toDouble()).toFloat()})
    val s = transform(r, { x: Float -> sin(x.toDouble()).toFloat()})
